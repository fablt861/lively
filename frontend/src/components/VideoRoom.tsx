    ...
    return (
        ...
        >
            ...
            <div className="flex-1 relative flex items-center justify-center overflow-hidden h-[100dvh]">
                    ...
                    <UnifiedAuthModal 
                        ...
                        onSuccess={(id, email, userRole, name, credits) => {
                            ...
                            if (!isConnected || isMatching) {
                                console.log("[Auth] Not in active call, triggering new joinQueue with proper ID");
                                // Small delay to ensure identity is processed and state is stable
                                setTimeout(() => {
                                    setHasStartedMatch(true); // Safety
                                    joinQueue();
                                }, 300);
                            }
                        }} />
                ...
                {/* User Credit Counter */}
                {role === "user" && (isConnected || isTeaserActive) && !showPaywall && !showAuthModal && userCredits !== null && (
                    <div className={`absolute top-16 left-6 md:top-6 md:right-6 md:left-auto z-30 flex items-center gap-3 px-4 py-2 sm:px-6 sm:py-3 bg-black/40 backdrop-blur-xl rounded-full border border-white/10 shadow-lg transition-all duration-700 overflow-hidden ${accountStatus !== 'guest' && userCredits <= 30 ? 'max-w-[calc(100vw-3rem)] md:max-w-2xl border-red-500/50 shadow-red-500/20' : 'max-w-fit'}`}>
                        <div className="flex items-center gap-2">
                            <span className="text-white/80 text-[10px] md:text-xs font-bold tracking-wider uppercase whitespace-nowrap hidden sm:block">{t('room.balance')}</span>
                             <span className={`font-mono text-xs md:text-lg font-bold whitespace-nowrap flex items-center gap-1.5 ${accountStatus !== 'guest' && userCredits <= 30 ? 'text-red-400 animate-pulse' : 'text-white'}`}>
                        ...
                        {accountStatus !== 'guest' && userCredits <= 30 && (
                ...
                {hasStartedMatch && !showPaywall && !showAuthModal && (
