...
module.exports = (io) => {
    ...
    router.post('/add-credits', async (req, res) => {
        ...
        try {
            ...
            // --- NEW: Real-time UI Sync ---
            if (io) {
                console.log(`[Socket] Emitting real-time credits_update to user id:${user.id}: ${newCredits}`);
                io.to(`id:${user.id}`).emit('credits_update', newCredits);
            }
